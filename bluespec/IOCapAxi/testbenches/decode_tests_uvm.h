...
template<class DUT>
class ManyLibRustRandomValidCaps_11 : public ManyRandomValidCaps_11<DUT> {
protected:
    virtual U128 random_cap(std::mt19937& rng) override {
        CCapU128 seed;
        U128::random(rng).to_le(seed);
        auto cap = CCap2024_11{};
        ccap2024_11_rand_valid_cap(&cap, &seed, nullptr, nullptr);
        return U128::from_le(cap.data);
    }
public:
    ManyLibRustRandomValidCaps_11(uint64_t count) : ManyRandomValidCaps_11<DUT>(count) {}
    virtual ~ManyLibRustRandomValidCaps_11() = default;

    virtual std::string name() override {
        return fmt::format("{} random valid 2024_11 caps generated by librust_caps_c", this->count);
    }
};

template<class DUT>
class ManyLibRustEdgeCaseCaps_11 : public ManyRandomValidCaps_11<DUT> {
    uintptr_t edge_case;
...
protected:
    virtual U128 random_cap(std::mt19937& rng) override {
        ...
        U128::random(rng).to_le(seed);
        auto cap = CCap2024_11{};
        ccap2024_11_rand_edge_case_cap(&cap, &seed, nullptr, nullptr, edge_case);
        return U128::from_le(cap.data);
    }
public:
    ManyLibRustEdgeCaseCaps_11(uint64_t count, uintptr_t edge_case) : ManyRandomValidCaps_11<DUT>(count), edge_case(edge_case) {}
    virtual ~ManyLibRustEdgeCaseCaps_11() = default;

    virtual std::string name() override {
        return fmt::format("{} random edge-case {} 2024_11 caps generated by librust_caps_c", this->count, ccap2024_11_rand_edge_case_str(edge_case));
    }
};
